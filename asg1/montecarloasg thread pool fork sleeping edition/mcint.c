...
void *SerialMonteCarlo()
{
    ...
    //this function in func.c returns the needed params 
    //for the object encasing the object whose integral we are trying to find
    ...
    while(1)
    {
        //if theres no work orders available, the thread goes to sleep
        //and increments the sleep threads counter
        while(!workhead)
        {
            pthread_mutex_lock(&wqlock);
            sleepthreads++;
            pthread_mutex_unlock(&wqlock);
            pthread_cond_wait(&cond, &wqlock);
        ...
        if(workhead)
        {
            ...
            if(workhead)
            {
                ...
                //this loop will generate random values for 
                //x, y, and z params that are inside the params given to us by
                //GetBoundaries and check if they are inside the params 
                //of the object who's integral we are looking for using IsInFunc
...
int main(int nargs, char **args)
{
    ...
    struct workq mywork;
    double calcms, xdist, ydist, zdist, xmax, ymax, zmax, xmin, ymin, zmin,
           compVol;
    ...
    //retrieve params for object surrounding the object we are finding 
    //the integral of
    GetBoundaries(&xmax, &xmin, &ymax, &ymin, &zmax, &zmin);
    ...
    //in the event that the number of iterations 
    //does not divide evenly over all threads
    //the remainder of the division is placed in n_extra 
    //that will distribute these extra iterations
    //to the threads
    n = N / p;
    n_extra = N % p;
    ...
    for(i=0;i<p;i++)
    {
        ...
        pthread_create(mythrs+i, &attr, SerialMonteCarlo, NULL);
    ...
    for(i=0;i<tst_num;i++)
    {
        ...
        //master thread will pause here until all worker threads have completed work
        while(hitcounter < p)
        {
            //if any threads are asleep, a wakeup signal is broadcasted
            if(sleepthreads > 0)
            {
        ...
        //the below calculates the best, worst, and average case for error and time of the work order
        avg_time += ms;
    ...
    //enqueue work order to kill threads
    for(i=0;i<p;i++)
    {
       mywork.workOrder = 0;
       AddWork(&mywork);
    }
    //if any threads are still sleeping when the work order to kill threads is handed out
